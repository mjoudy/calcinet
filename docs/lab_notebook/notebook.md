...
(`results/ledger.csv`, one row per run; rebuild/query with `src/calcinet/ledger_cli.py`).
...
`calcinet.connectivity.solvers.from_moments`. No resimulation, no cluster — solved
...
4. **R.2 is the only figure in the whole project that models a realistic camera
   at all.** Checked `calcinet/simulation/calcium_signal.py`, `wrapup_run.py`,
...
**PR-AUC added (zero new compute):** `src/calcinet/connectivity/analyze_run.py`'s `score()` has
...
**Method:** `src/calcinet/simulation/ou_linear_ground_truth.py`. Same connectivity as
...
`src/calcinet/io/streaming.py`) landed far from the true value
...
### 2026-08-25 — R.4 20M-ms extension lands (N=2500, R.5 N=12500); why the seed-error band only shows up at long T; Dale's real, modest effect at large N

**What ran:** the N=2500/N=5000/N=12500(R.5-only) 20M-ms extension jobs from
2026-08-14 (see [[hpc_deployment_plan]]) were finally checked, fixed, and
pulled back. N=2500 now has the full 5-seed 10-point grid to 20M ms (matching
N=1250); R.5 now has real N=12500 data (previously died 20% into its first
checkpoint). N=5000's extension is still running as of this entry. Practical
bugs found along the way (for future reference, not re-litigated here): the
first N=5000 attempt requested `--mem=512G` — the *exact* full RAM of an
`l40s` node (`sinfo`: 512000 MB) — which SLURM rejects outright (no OS
headroom), producing an instant `FAILED` with no log at all; already fixed to
400G. `fig_r4_plot.py`/`fig_r5_plot.py` regenerated; `fig_R5_length.png`
added (T/N and raw-ms versions of the R.5 crossover figure, mirroring
fig_R4/fig_R4_TN — zero new compute, same CSVs, just an `xkey` parameter).

**Mechanism — why the shaded ±1 SD seed-band is invisible almost everywhere
and then suddenly visible at the tip of exactly the curves that reach 20M
ms** (measured on excitatory recall, N=1250 and N=2500, 5 seeds each):

| N | T=100k std | T=1M std | T=5M std | T=10M std | T=20M std |
|---|---|---|---|---|---|
| 1250 | 0.004 | 0.005 | 0.004 | 0.012 | **0.036** |
| 2500 | 0.002 | 0.003 | 0.005 | 0.015 | **0.041** |
| 12500 | 0.0002 | 0.0008 | 0.0006 | 0.001 | *(capped at 10M, not run)* |

Cross-seed spread is flat and tiny (~0.001-0.005) everywhere the curve is
still climbing, then jumps 5-10x specifically past ~10M ms — i.e. only on
the two curves (N=1250, N=2500) that have been run out that far. **Bias-
variance reading, ties directly to the existing "more data fixes noise, not
confounding" framing ([[shared_input_findings]], 2026-08-07 entry):** while
recall is still climbing with T, that systematic, T-driven improvement
dominates and every seed tracks it almost identically (small band). Once the
curve goes flat (past ~5M ms), the *systematic* component has bottomed out
and what's left in the residual is genuine seed-to-seed network-realization
variability — always present, just invisible earlier because it was small
relative to the still-shrinking bias term. Concretely: N=2500 seed 5 landed
at E_rec=0.627 at T=20M vs ~0.72 for its 4 siblings — a real outlier
realization, not solver noise (correctness of the pipeline isn't in
question; this is exactly what n=5 seeds is for). One practical
consequence: **error bars on a scaling-law figure should be expected to
widen near the plateau, not shrink** — a flat mean with a widening band is
the signature of "systematic effect exhausted, only realization variance
left," worth stating explicitly if this figure goes in the report so it
doesn't read as noisy/suspicious.

**Dale's-law regularization at large N — corrected, quantified (previous
same-day chat claim of a "real, substantial" E-recall benefit was too
strong; numbers below are the actual best-over-lambda values from
`r5_<net>.csv`):**

| net (largest T/N) | OLS corr / E_rec | EN+Dale corr / E_rec | Δcorr | ΔE_rec |
|---|---|---|---|---|
| N=1250 (T/N=160000) | 0.775 / 0.783 | 0.847 / 0.793 | **+0.07** | +0.01 |
| N=12500 (T/N=8000) | 0.859 / 0.522 | 0.942 / 0.547 | **+0.08** | +0.03 |

Dale's-law-constrained regularization gives a real, sizable lift to overall
**correlation** at large N (+0.07 to +0.08) but only a small lift to
**excitatory recall specifically** (+0.01 to +0.03) — consistent with, not a
contradiction of, the established "thresholding/regularization cannot fix
confounding" result ([[shared_input_findings]]): correlation is dominated by
inhibition + overall fit (a variance-limited quantity that regularization
legitimately helps), while excitatory recall is the bias-limited,
confounding-capped quantity that no amount of regularization strength can
move much. Report framing: Dale's law is a real, worthwhile addition to the
pipeline, but not a fix for the excitatory ceiling — don't oversell it as
"closing the gap."

**Open, deliberately deferred:** N=12500 is missing seeds 4-5 specifically
at T=7.5M/10M (3/5 seeds there, 5/5 everywhere shorter) — likely that
seed's array task hit its 8h limit or OOM'd. Not pursued (2026-08-25
decision) — noted here only so a future "why does N=12500's band look
different at its tail" question isn't re-investigated from scratch.
