...
\emph{linear regression}. We start from one simple idea --- a neuron's
activity is well predicted by the recent activity of the other neurons --- and
build the regression up one step at a time, from a single neuron to the whole
network. We then show how the regression is actually solved, and how three
things change the picture: the \emph{external input}, the meaning of the
\emph{residual}, and the use of \emph{regularization}. No specialist
...
% ======================================================================
\section{From an idea to a regression, step by step}
\label{sec:steps}

\paragraph{The idea.}
A neuron's activity in the next moment is well predicted by the recent activity
of the neurons that connect to it: more drive from the neurons that excite it
means more activity; more drive from the neurons that inhibit it means less. We
turn this directly into a \emph{discrete-time} model --- one time step to the
next --- and build it up from a single neuron to the whole network. We use the
simplest version: each neuron at time $t$ depends on all neurons at time $t-1$
(a one-step lag).
...
plus leftovers}. This is the discrete-time model we will fit.
...
The estimate gives the right \emph{sign} and \emph{relative} strength of
connections --- this is \emph{effective} connectivity. It is not necessarily
on the same absolute scale for every neuron: how strongly a neuron's own
activity converts into influence on its neighbours can differ from neuron to
neuron, so two entries of $\widehat{\Amat}$ with the same true synaptic weight
need not come out equal. Correcting for that per-neuron scale (rescaling /
balance corrections) is what would be needed to move from \emph{effective} to
\emph{raw synaptic} weight. When ground truth is available, the quality is
scored by how well $\widehat{\Amat}$ matches the true matrix (e.g.\
...
\[
\begin{aligned}
\text{connectivity} \;\to\; \text{activity} \;\to\; \text{calcium}
\;&\xrightarrow{\text{deconvolve}}\; \text{feed }\xx\\[-2pt]
\xrightarrow{\text{regression}}\; \widehat{\Amat} \;&\to\; \text{connectivity estimate.}
\end{aligned}
\]

\paragraph{The assumptions, in one place.}
\begin{enumerate}
\item Next-step activity is approximately a linear function of current activity
(a reasonable approximation for small fluctuations around a stable, irregular
operating state).
