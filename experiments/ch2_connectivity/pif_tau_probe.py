...
example"). Same purpose as experiments/ch2_connectivity/r4_tune_regime.py's N sweep, but the knob
...
r4_tune_regime.py; sparse events only, dense (N,T) never built):
  srun --cpus-per-task=8 --mem=16G --time=00:30:00 \\
    $(ws_find calcium_ar)/env/bin/python experiments/ch2_connectivity/pif_tau_probe.py --stage x100

Usage:
  python experiments/ch2_connectivity/pif_tau_probe.py --stage x100    # tau_m=2000ms   (n1250_pif x10)
  python experiments/ch2_connectivity/pif_tau_probe.py --stage x1000   # tau_m=20000ms
  python experiments/ch2_connectivity/pif_tau_probe.py --stage x10000  # tau_m=200000ms -- read the caution above first
  python experiments/ch2_connectivity/pif_tau_probe.py --tau-m 2000 --etas 20 30 45 65 90 120 160 200
"""
...
# n1250_r4 / n1250_pif's fixed parameters (see NETS in experiments/shared/wrapup_run.py) --
