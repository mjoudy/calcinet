...
sys.path.insert(0, str(ROOT)); sys.path.insert(0, str(ROOT / "research" / "experiments" / "shared"))
...
def main():
    ...
    ap.add_argument("--fresh", action="store_true",
                    help="ignore gt_cache and run a fresh BrunelNetwork simulation "
                         "instead (seed=1) -- use this if the cached ground truth "
                         "might be stale relative to the current config for --net")
    ap.add_argument("--seed", type=int, default=1, help="only used with --fresh")
    args = ap.parse_args()
    ...
    if args.fresh:
        ...
        print(f"running a FRESH {args.net} simulation (seed={args.seed}, "
              f"bypassing gt_cache) ...", flush=True)
        net = BrunelNetwork(n_excitatory=cfg["n_excitatory"], n_inhibitory=cfg["n_inhibitory"],
                            epsilon=cfg["epsilon"], g=cfg["g"], eta=cfg["eta"],
                            J_ex=cfg["J_ex"], delay=cfg["delay"], V_reset=cfg["V_reset"],
                            sim_time=args.checkpoint_ms, dt=dt,
                            n_threads=cfg.get("n_threads", 8), seed=args.seed)
        net.build(); net.run(densify=False)
        idx, tms = net.get_spike_events()
        spikes = (idx.astype(np.int64), tms)
        adj = net.get_adjacency()
        np.fill_diagonal(adj, 0.0)
        print(f"fresh {args.net}  N={N}  T={args.checkpoint_ms/1000:.0f}k ms")
    else:
    ...
    # --fresh writes to a DIFFERENT file (never overwrites the cache-based
    # preproc_<net>.npz) so the old (possibly stale) result stays available
    # for comparison rather than being silently replaced.
    suffix = "_fresh" if args.fresh else ""
    out_path = out_dir / f"preproc_{args.net}{suffix}.npz"
