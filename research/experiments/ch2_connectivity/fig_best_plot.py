...
Diverging colour: red = excitatory (+), blue = inhibitory (-). Each panel gets its
OWN scale per side (excitatory vmax from the positive values, inhibitory vmax from
the negative ones) via TwoSlopeNorm, not one shared symmetric vmax — ground truth
inhibitory weights run ~8x larger in magnitude than excitatory here (Dale g-scaling),
so a single symmetric scale washes every excitatory edge out to near-white.
...
Usage:
  python research/experiments/ch2_connectivity/fig_best_plot.py --data ~/calcium_results/best/best_n12500lr.npz \
...
def two_slope_norm(M, pct):
    """Independent vmax per sign, so excitatory/inhibitory each use the full
    colour range even when one is much larger in magnitude than the other
    (Dale g-scaling: inhibitory weights here run ~8x excitatory)."""
    pos, neg = M[M > 0], M[M < 0]
    vmax_pos = np.percentile(pos, pct) if pos.size else 1e-6
    vmax_neg = np.percentile(-neg, pct) if neg.size else 1e-6
    return TwoSlopeNorm(vmin=-(vmax_neg or 1e-6), vcenter=0, vmax=(vmax_pos or 1e-6))


def show(fig, ax, M, pct, title, ylabel=None, sub_split=None):
    norm = two_slope_norm(M, pct)
