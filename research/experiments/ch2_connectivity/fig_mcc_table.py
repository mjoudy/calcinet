...
Reads the metrics.csv files written by src/calcinet/connectivity/analyze_run.py (which now carries
...
Usage:
  python research/experiments/ch2_connectivity/fig_mcc_table.py --root results --method ols --tpick longest
  python research/experiments/ch2_connectivity/fig_mcc_table.py --root results --tpick matched-tn --tn 4000
"""
...
def main():
    ...
    if not rows:
        raise SystemExit("no re-scored metrics.csv found (need the mcc3 column). "
                         "Run src/calcinet/connectivity/analyze_run.py on the wrap-up dirs first.")
