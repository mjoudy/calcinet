...
Reads the small r1_data.npz written on the cluster and renders the 2x3 panel:
    ...
    [ 3x3 confusion, R+C ]   [ ROC (+AUC) ]   [ PR (+AP) ]
...
Usage:
  python research/experiments/ch2_connectivity/fig_r1_plot.py --data results/fig_data/r1_data.npz \
...
def draw_confusion(ax, cm):
    """Row-normalized color (recall direction), but each cell is annotated
    with BOTH row value (P(pred|true), recall on the diagonal) and column
    value (P(true|pred), precision on the diagonal) — so precision and
    recall are both visible without needing a second matrix."""
    row_norm = cm / np.clip(cm.sum(1, keepdims=True), 1, None)
    col_norm = cm / np.clip(cm.sum(0, keepdims=True), 1, None)
    ax.imshow(row_norm, cmap=fs.BLUE_SEQ, vmin=0, vmax=1, aspect="equal")
    for i in range(3):
        for j in range(3):
            light = row_norm[i, j] > 0.55
            color = "white" if light else fs.INK
            ax.text(j, i - 0.14, f"R {row_norm[i, j]:.2f}", ha="center",
                    va="center", color=color, fontsize=8.7)
            ax.text(j, i + 0.14, f"C {col_norm[i, j]:.2f}", ha="center",
                    va="center", color=color, fontsize=8.7)
    ax.set_xticks(range(3)); ax.set_yticks(range(3))
    ax.set_xticklabels(CLASS_NAMES); ax.set_yticklabels(CLASS_NAMES)
    ax.set_xlabel("predicted"); ax.set_ylabel("true")
    ax.set_title("confusion — R: P(pred|true)=recall, C: P(true|pred)=precision")
    fs.despine(ax, keep=())
