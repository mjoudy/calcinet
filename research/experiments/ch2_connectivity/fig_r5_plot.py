...
Two figures from the r5_<net>.csv files:
  ...
  fig_R5_lambda   the lambda LANDSCAPE. metric vs lambda/lambda_max for the
                  ...
                  One representative (the smaller) network, two measures only
                  (correlation, excitatory recall) -- the full measure x
                  network comparison is redundant with fig_crossover above, so
                  this stays a single illustrative case (see LAMBDA_MEASURES).
...
Usage:
  python research/experiments/ch2_connectivity/fig_r5_plot.py --root results/r5 --nets n1250r4 n12500r4 \
      --out figures/fig_R5
"""
...
MEASURES = [("corr", "correlation"), ("E_rec", "excitatory recall"),
            ("roc_auc", "ROC-AUC (connected vs. none)"), ("pr_ap", "PR-AUC (connected vs. none)")]
# The lambda-landscape figure (fig_lambda) shows only these two -- the other
# two measures track E_rec closely there and just add redundant panels -- and
# only for one (the smaller) network, as a single illustrative case; the full
# measure x network comparison already lives in fig_crossover.
LAMBDA_MEASURES = [("corr", "correlation"), ("E_rec", "excitatory recall")]
...
def best_vs_x(rows, method, key, xkey):
    """-> (x[], best metric over lambda[]) for one estimator, x = row[xkey]."""
    by_x = {}
    for r in rows:
        if r["method"] != method:
            continue
        ...
        if not np.isfinite(v):
            continue
        x = float(r[xkey])
        by_x[x] = max(by_x.get(x, -np.inf), v)
    xs = sorted(by_x)
    return np.array(xs), np.array([by_x[x] for x in xs])


def fig_crossover(data, out, xkey="TN", xlabel="samples per neuron   T / N",
                   title_suffix="does regularization lead at small T/N?"):
    nets = list(data)
    ...
    for c, net in enumerate(nets):
        ...
        for r, (key, label) in enumerate(MEASURES):
            ...
            for m in METHOD_ORDER:
                x, val = best_vs_x(rows, m, key, xkey)
                if len(x) == 0:
                    continue
                ax.plot(x, val, "o-", color=METHOD_COLOR[m], lw=2.2 if m == "ols"
    ...
    fig.suptitle(f"Recovery vs data, each estimator at its best λ — {title_suffix}",
                 fontsize=13, color=fs.INK, x=0.08, ha="left", y=0.965)
    fs.save(fig, out)


def fig_lambda(data, out, method="en", nets=None):
    """One representative case, not the full measure x network sweep -- see
    fig_crossover for that. `nets` defaults to every net in `data`; the caller
    passes a single-net list to keep this to one illustrative example."""
    nets = nets if nets is not None else list(data)
    # +2.2in of width reserved for an OUTSIDE legend -- with only one T/N-colored
    # line per (row, column) previously hidden behind the in-axes legend box at
    # the low-lambda end where most curves are flat and close together.
    fig, axes = plt.subplots(len(LAMBDA_MEASURES), len(nets),
                             figsize=(6.4 * len(nets) + 2.2, 3.6 * len(LAMBDA_MEASURES)), squeeze=False)
    fig.subplots_adjust(left=0.08, right=1 - 2.2 / (6.4 * len(nets) + 2.2), top=0.9,
                        bottom=0.09, hspace=0.28, wspace=0.2)
    for c, net in enumerate(nets):
        rows = data[net]
        N = int(rows[0]["N"])
        tns = sorted({float(r["TN"]) for r in rows})
        colors = plt.cm.viridis(np.linspace(0.15, 0.85, len(tns)))
        for r, (key, label) in enumerate(LAMBDA_MEASURES):
            ax = axes[r][c]
            ...
            if r == 0:
                ax.set_title(f"N = {N}")
    axes[0][-1].legend(fontsize=9, loc="upper left", bbox_to_anchor=(1.02, 1.0),
                       borderaxespad=0, title="solid = EN\ndotted = OLS")
...
def main():
    ...
    fig_crossover(data, args.out, xkey="TN", xlabel="samples per neuron   T / N",
                  title_suffix="does regularization lead at small T/N?")
    fig_crossover(data, args.out + "_length", xkey="T_ms", xlabel="recording length (ms)",
                  title_suffix="does regularization lead at short recordings?")

    # lambda landscape: one representative (smaller) network only
    small_net = min(data, key=lambda n: int(data[n][0]["N"]))
    fig_lambda(data, args.out + "_lambda", nets=[small_net])
