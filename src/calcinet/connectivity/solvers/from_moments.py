...
src/calcinet/io/streaming.py) and then solve here with O(N^2) RAM.
...
def fista_from_moments(Cxx: np.ndarray, Cyx: np.ndarray, lam_l1: float,
    ...
    calcinet/solvers/fista.solve but on precomputed moments."""
    N = Cxx.shape[0]
...
def dale_from_moments(Cxx: np.ndarray, Cyx: np.ndarray, types: np.ndarray,
    ...
    calcinet/solvers/dale_fista.dale_fista but on precomputed moments."""
    N = Cxx.shape[0]
