...
-----
    ...
    python -m calcinet.ledger_cli rebuild
    python -m calcinet.ledger_cli rebuild --results results --out results/ledger.csv

    # Quick "what does this parameter do?" — averaged over everything else
    python -m calcinet.ledger_cli effect lag_ms
    python -m calcinet.ledger_cli effect smooth_window_ms --metric connectivity/auc_roc

    # Dump the whole ledger to the terminal
    python -m calcinet.ledger_cli show
    python -m calcinet.ledger_cli show --cols lag_ms,smooth_window_ms,connectivity/pearson
"""
...
from calcinet.framework.ledger import rebuild, effect_of
